...
export function addTrees(containBK,trees ,screenwidth, screenheight)
{
    // Width of each tree.
    const treeWidth = 200;
    ...
    const count = screenwidth / (treeWidth + spacing) + 1;

    // Create an array to store all the trees.
    
    let containsTree = new Container();
    ...
    for (let index = 0; index < count; index++)
    {
        ...
        containsTree.addChild(tree);
        trees.push(tree);
    }

    containBK.addChild(containsTree);
}
